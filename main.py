...
def index():
    ...
    if user:
        ...
        return render_template('usersPage.html',tab_title="User's Blog Page (get)",posts=posts)
    else:
...
def signup():
    if request.method == 'POST':
        ...
        user_exists_error = 'This user already exists, Please Login.'
...
def main_page():
    ...
    if blog_id != None:
        ...
        user_obj = User.query.filter_by(id=entry.author_id).first() 
    ...
    if user == None:
        ...
        posts = Blog.query.filter_by(author_id=user_obj.id).all()
